...
public class TF_IDF {
    ...
    private final long TOTAL = 100000000;//百度搜索出的最大数量为1亿
    ...
    public List<String> keywords(String text,int topN){
        ...
        for (String key : segMap.keySet()) {
            ...
            double tfidf = getTFIDF(key,segMap.get(key));
            tfidfMap.put(key,tfidf);
        }

        // 3. 取 TopN
        List<String> keywords = getTopN(tfidfMap,topN);
        return keywords;
    }
